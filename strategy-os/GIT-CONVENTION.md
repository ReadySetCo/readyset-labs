...
8. **Cambios grandes = rama + worktree (desde 2026-09-10):** para probar un rediseño sin tocar main, `git worktree add ../strategy-os-<tema> -b <tema>-redesign` — se itera y commitea ahí (prefijo del chat + tema), el browser prueba esa copia, y al aprobar se hace `git merge` en main (y recién entonces sync a Labs + deploy). Si se descarta: `git worktree remove` + borrar la rama. NUNCA sync/deploy desde una rama sin mergear.
